...
while True:
    level+= 1
    print('Level %d' %(level))
    def userlevboost(stat):
        stat = stat + ((userlevel - 1) * 0.10 * stat)
        return int(stat)

    class charClass:
        def __init__(self, namecall, hit_points, mana_points, str, spd, mag, dfense, turn_num, turn_wait):
            self.name = namecall
            self.hp = userlevboost(hit_points)
            self.mp = userlevboost(mana_points)
            self.strength = userlevboost(str)
            self.speed = userlevboost(spd)
            self.magic = userlevboost(mag)
            self.defense = userlevboost(dfense)
            self.turn = turn_num
