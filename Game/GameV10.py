...
class charClass:
    ...
    def getitem(self, target):
        item_roll = random.randint(0, 100)
        global potionInv
        global phoenixdownInv
        itemgotten = ""
        numgotten = 0
        if (item_roll >= 0 and item_roll <20):
            itemgotten = "potions"
            numgotten = 2
            potionInv += 2
        if (item_roll >= 20 and item_roll <70):
            itemgotten = "potion"
            numgotten = 1
            potionInv += 1
        if (item_roll >= 70 and item_roll <90):
            itemgotten = "phoenix downs"
            numgotten = 2
            phoenixdownInv += 2
        if (item_roll >= 90 and item_roll <= 100):
            itemgotten = "phoenix down"
            numgotten = 1
            phoenixdownInv += 1
        print("%s retrieved %d %s" %(self.name, numgotten, itemgotten))
        self.turnwait -= 8
    ...
    '''
        ...
        if u2choice == 'knucks':
            ...
            if u2 == u1:
                ...
                break
    '''       
    print("You can 'getitem' at any time")
